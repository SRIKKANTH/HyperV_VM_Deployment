...
function UpdateState([System.Xml.XmlElement] $vm, [string] $newState)
{
    ...
    #$vm.stateTimestamp = [DateTime]::Now.ToString()
    if(Get-Member -inputobject $vm -name "stateTimestamp" -Membertype Properties)
    {
        $vm.stateTimestamp = $(Get-Date).ToString()
    }
    else
    {
        $vm | Add-Member -NotePropertyName stateTimestamp -NotePropertyValue $(Get-Date).ToString()
    }
    return $vm.state
}
